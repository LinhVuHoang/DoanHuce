 ...
 // Mã secretKey này phải được bảo mật tuyệt đối, các bạn có thể lưu vào biến môi trường hoặc file
 const refreshTokenSecret = process.env.REFRESH_TOKEN_SECRET || "refreshtoken-DoanHUCE";
 /**
  * controller login
  * @param {*} req 
  * @param {*} res 
 ...
 let login = async (req, res) => {
   dataaccount= await model.getdata(req.body)
  debug(`ko lỗi ${JSON.stringify(dataaccount)}`)
    try {
 
    if(dataaccount ==null){
      console.log('error');
    }
     debug(`Đang giả lập hành động đăng nhập thành công với Email: ${req.body.Username} và Password: ${req.body.Password}`);
    ...
    //  };
     ...
     const refreshToken = await jwtHelper.generateToken(dataaccount, refreshTokenSecret, refreshTokenLife);
    ...
    console.log(error);
    debug(`Sai tài khoản hoặc password`);
